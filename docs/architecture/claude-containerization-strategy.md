...
#### C. Claude 身份强行同步 (Force-Sync & Volatile)
*   **原则**：容器每次重启时，都会强制从只读种子 `/home/hotplex/.claude.json.seed` 覆盖拷贝到 `~/.claude.json`。
*   **目的**：确保 Bot 始终获得宿主机最新的 `mcpServers` 配置，防止因具名卷残留导致的配置污染（Stale Data）。
*   **隔离**：拷贝发生在容器的可写层，因此每个 Bot 实例拥有独立的运行期标识。
...
| 路径 | 存储介质 | 角色 | 读写属性 | 持久性 |
| :--- | :--- | :--- | :--- | :--- |
| `/home/hotplex/.hotplex/configs` | 绑定挂载 (Bind Mount) | 全局配置模板 | RO (经 Entrypoint 处理) | 宿主机维护 |
| `/home/hotplex/.claude/` | **具名卷 (Named Volume)** | 插件、日志、临时状态 | **RW (实例隔离)** | **卷持久化** |
| `/home/hotplex/projects/` | 绑定挂载/具名卷 | 代码项目 | RW | 持久 |
| `~/.claude.json` | **容器可写层** | 核心身份配置 | **RW (实例隔离)** | **挥发性 (随重启重置)** |

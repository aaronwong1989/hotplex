# 🔀 v0.18.x 全局升级与迁移指南 (Global Upgrade & Migration Guide)

随着 HotPlex 迈向更加系统化与模块化的 v0.19.0 版本，我们在 `v0.18.x` 系列更新中引入了一些关键的架构优化和规范调整。

本手册旨在帮助您将现有的部署环境、`.env` 文件平滑迁移至新规范。

---

## 🔒 1. 核心环境变量前缀规范化 (Environment Variables)

为了彻底解决在多服务容器部署环境中可能出现的变量命名冲突（例如与常见框架或系统的 `PORT`、`LOG_LEVEL` 冲突），我们对所有核心环境变量实施了统一的命名空间隔离。

**自 v0.18.2 起，几乎所有 HotPlex 依赖的环境变量都必须以 `HOTPLEX_` 作为前缀。**

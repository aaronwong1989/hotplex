...
**版本**: v0.36.0 | **核心角色**: AI Agent 执行运行时协议 (Cli-as-a-Service)
...
- **`cmd/`**：守护进程与 CLI 的入口。包含各个命令的子包。
- **`engine/`**：公开的执行运行器 (`Engine`)。编排 Prompt 执行、安全 WAF 审计及事件分发。
- **`provider/`**：不同 AI CLI 智能体的抽象层。包含 `Provider` 接口及 `claude-code`、`opencode`、`opencode-server`、`pi` 的具体实现。
- **`brain/`**：Native Brain — 编排、路由与记忆压缩。包含 `brain/llm/` 用于 LLM 驱动的智能处理。
- **`cache/`**：响应缓存层，用于减少冗余 AI 调用。
- **`types/`**：基础数据结构 (`Config`, `StreamMessage`, `UsageStats`)。
- **`event/`**：统一事件协议与回调定义 (`Callback`, `EventWithMeta`)。
- **`hooks/`**：事件钩子系统，支持 Webhook 及审计通知接收器 (Slack、通用 Webhook)。
- **`chatapps/`**：**平台接入层**。将 HotPlex 连接到社交平台，支持多适配器架构：
  ...
  - 核心组件：
    ...
    - `session/`：会话状态管理
    - `internal/`：平台内部工具
- **`plugins/storage/`**：消息持久化后端 (SQLite、PostgreSQL)。
- **`sdks/`**：第三方集成客户端 SDK — Python SDK (`sdks/python/`) 与 TypeScript SDK (`sdks/typescript/`)。
- **`scripts/`**：Shell 脚本与运维工具（资源生成、Git Hooks、测试、Provider 切换）。
- **`install/`**：一键安装脚本（`hotplex-install.sh` 适用于 Unix，`hotplex-install.ps1` 适用于 Windows）。
- **`_examples/`**：示例实现，涵盖 Go、Python、TypeScript、Java 及 Node.js 集成。
- **`docker/`**：Dockerfile 与容器编排配置。
- **`configs/`**：支持继承机制的 SSOT 配置模板。
- **`docs/`**：架构文档与设计参考。
- **`docs-site/`**：项目文档网站源码。
...
- **`internal/config/`**: 具备文件监听功能的配置热加载。
- **`internal/admin/`**: **管理平面 (Management Plane)**。提供 Admin API (9080 端口)，用于远程管理会话、诊断及配置校验。
- **`internal/adminapi/`**: Admin API HTTP 处理器，负责认证、领域逻辑与请求路由。
- **`internal/agent/`**: 智能体注册表，基于卡片的智能体管理。
- **`internal/brain/`**: Brain 内部实现 — Cron 意图处理与调度。
- **`internal/bridgewire/`**: Bridge Wire 通信协议，用于跨进程协调。
- **`internal/permission/`**: 权限管理，基于匹配器的访问控制与策略存储。
- **`internal/panicx/`**: Panic 恢复工具，保障 Goroutine 安全。
- **`internal/cron/`**: **自动化引擎 (Automation Engine)**。负责调度 AI 定时任务并维护执行历史。
- **`internal/relay/`**: **智能体通讯 (Inter-Agent Communication)**。处理跨平台的 Bot-to-Bot 消息路由与中继。
- **`internal/secrets/`**: 密钥提供者，用于 API Key 管理与凭证轮换。
- **`internal/telemetry/`**: OpenTelemetry 集成，提供链路追踪与指标采集。
- **`internal/strutil/`**: 高性能字符串处理与路径清理工具。
...
HotPlex v0.33.0 引入了**单一可信源 (Single Source of Truth, SSOT)** 配置架构，采用集中式基础模板与继承机制。
...
1.  **SSOT 基础模板**：所有配置继承自 `configs/base/` 中的模板
2.  **环境变量管理敏感信息**：凭证存储在 `.env` 文件中，通过 `${VAR}` 语法引用
3.  **配置继承机制**：使用 `inherits: ./path/to/parent.yaml` 扩展基础配置
4.  **实例隔离**：每个机器人部署应有独立的配置目录
...
> **注意**：`configs/chatapps/` 目录已废弃。请使用 `configs/base/` 作为所有基础模板。
...
*   **支持的 Provider**：Claude Code (默认)、OpenCode、OpenCode Server (HTTP 传输)、Pi。
...
*   **链路追踪与指标**: 内置 OpenTelemetry，并暴露 Prometheus 兼容的 `/metrics` 接口。
*   **定时任务与中继**: 新增的内部服务，用于异步自动化任务和跨平台消息路由。
...
- [x] **双管理模式**: 提供直接的本地 CLI 访问及远程 Admin API (9080 端口)
- [x] **Cron 调度系统**: 支持带失败恢复和历史记录的 AI 定时任务
- [x] **Bot 中继机制**: 具备断路器保护的跨平台消息路由处理
